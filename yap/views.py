...
class HomePageStatistics(APIView):
    ...
    @staticmethod
    def get_users_stats(request,  amount=5, _time=1000000, country=0, state=0, city=0, gender="n", min_age=0, max_age=99):
        data = OrderedDict()
        users = User.objects.using('ye_1_db_1').all()
        time = datetime.datetime.now() - datetime.timedelta(minutes=_time)
        users_count = len(users)
        if users:
            data['Total Number of Active Users'] = users.filter(is_active=True)
            data['Total New Users'] = users.filter(date_joined__gte=time)
            # Get top [amount]
            d_countries = {}
            if _time == 1000000:
                ...
                for u in users:
                    ...
                    if last and (last.session_device_token == "<>" or not last.session_device_token or not last.is_active):
                        continue
                    data['Users to pushnotify'].append(u)
            else:
                data['Top Countries Users'] = None
                data['Birthdays this Month'] = None
                data['Birthdays Today'] = None
                data['Users to pushnotify'] = None
